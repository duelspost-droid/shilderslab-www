...
> 5. ~~Supabase 리전 확인~~ **해소(2026-08-09)** — 지연 측정으로 US East 확인.
>    현행 고지 `이전 국가: 미국` 이 맞아 **수정 불필요**. 근거는 4항 P2.
> 6. ~~백그라운드 태스크 4건~~ **종결(2026-08-23)** — admin CSS 분리✅·CSP unsafe-inline 제거✅ 완료,
>    SDK 페이지별 분리⛔(순이득 마이너스)·폰트 셀프호스팅⛔(오너: 현행 유지)로 결정. 4항 "배경 리팩터" 참조.
...
- ~~**P2** Supabase 리전 확인~~ → **해소 (2026-08-09, 지연 측정으로 확정에 준하는 근거 확보)**
  대시보드가 계속 로딩 스켈레톤이라 **대시보드 없이** 왕복 지연으로 판정했다.
  결론: **US East(us-east-1) 계열** → 현재 고지의 `이전 국가: 미국` 이 **실제와 맞다. 수정 불필요.**

  측정 방법과 값(TLS 수립 이후 시간만, 각 8회 중앙값):
  | 대상 | 값 |
  |---|---|
  | Supabase 게이트웨이(원본 미도달) | 16.9ms |
  | Supabase auth health(원본 도달) | 24.9ms |
  | **Supabase REST 질의(원본+DB)** | **195.8ms** |
  | 대조군 S3 서울 ap-northeast-2 | 9.0ms |
  | 대조군 S3 도쿄 ap-northeast-1 | 65.9ms |
  | 대조군 S3 싱가포르 ap-southeast-1 | 100.6ms |
  | **대조군 S3 버지니아 us-east-1** | **195.8ms** |
  | 대조군 S3 오레곤 us-west-2 | 138.4ms |

  질의 크기를 바꿔도 값이 그대로였다(1행 195.1 / 전체행 187.1 / 조인 205.4) →
  **시간의 대부분이 질의 처리가 아니라 왕복 거리**임을 확인했다. 아시아 리전(9~101ms)과는
  자릿수가 다르고 버지니아와 일치한다.
  ⚠ 이것은 **추론**이지 대시보드 확인은 아니다. 다만 **결론이 현행 고지와 같으므로 법적 고지를
  바꾸지 않는다** — 바꿀 이유가 생겼을 때만 대시보드 확인이 필요하다.
  (대시보드가 뜨면 Settings › General 의 Region 으로 최종 확인해 이 항목을 확정 처리할 것.)
...
- ⛔ **supabase-js(137KB) 페이지별 로드 — 안 함(2026-08-23 결정, 순이득 마이너스)**. site.js 가
  하이드레이션·설정·공지배너·방문로깅에 SDK 를 **전 페이지에서** 쓴다. 특히 페이지 문구 즉시 반영은
  오너가 명시적으로 원한 기능이라, SDK 를 콘텐츠 페이지에서 빼면 그게 깨진다. 안전하게 하려면
  데이터 레이어 전부(하이드레이션+설정+로깅+esc/md 헬퍼)를 raw fetch 로 재작성해야 하는데,
  SDK 는 defer+캐시라 실이득이 작다. **비용>이득이라 종결.** 되살릴 거면 raw fetch 재작성부터.
- ⛔ **Google Fonts 셀프호스팅 — 안 함, 현행 유지(2026-08-23 오너 결정)**. 폰트 CDN 은 한국
  실무상 처리위탁으로 보지 않아 방침 위탁표에서 뺀 현 상태로 충분하다는 판단. 셀프호스팅 시
  IBM Plex Sans KR(한글)은 풀 woff2 가 수 MB라 용량이 급증하고, 서브셋 파이프라인은 글리프 누락
  시 □ 위험이 있다. 추가 작업·용량·위험 대비 이득이 작아 **Google Fonts CDN 그대로 유지**.
...
| 2026-08-08 | **관리자 콘솔 모바일 수정** — 상단바 `height:64px` 고정이 wrap 된 줄을 탭 위로 흘려보내던 문제(`min-height` 로 교체) · 버튼 54→33px · 탭 스트립 오른쪽 페이드 추가. 390/360px 검증 | — |
