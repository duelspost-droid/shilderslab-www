...
| 페이지 문구 CMS (`0005`) | ⚠️ **코드 완료 · 라이브 미적용** — 오너가 `0005` 를 실행해야 관리자 콘솔 [페이지 문구] 탭이 동작한다(3항 ⑦) |
...
- `0005_shilderslab_content.sql` — ⏳ **미적용. 오너가 SQL 에디터에서 실행해야 한다.**
  페이지 문구 CMS(`sl_content`). 적용 전까지 관리자 콘솔 **[페이지 문구]** 탭은 "불러오지 못했습니다"를 띄우고,
  공개 페이지는 코드에 박힌 기본 문구로 그대로 뜬다(장애 아님).
  적용 후 확인:
  ```sql
...
관리자 콘솔 → **[페이지 문구]** 탭. 편집 가능한 18개 블록:
회사소개 4 · 대표이사 인사말 4 · 서비스 2 · 푸터 1 · 법인 정보 7.
...
**3) CI 심볼 재검토 (오너 판단 대기) — 비교 시트 제시함 (2026-07-30)** — 현재 심볼은 실드 + **'S' 각인**이다.
...
16/32/64px + 역상 + 브라우저 탭 시뮬레이션까지 렌더한 비교 시트를 만들어 오너에게 제시했다(확정 대기).
생성 스크립트는 스크래치패드에 두었다 — 실드 패스·폰트·컬러는 `build-ci.py` 와 동일하고 각인 글자만 바꾼다.
실측: 각인 잉크 폭/높이 = S 22.6/28.6 · U 22.4/28.1 · US 29.2/17.2(64 그리드).
US 는 cap 을 16.5 로 줄여야 들어가고, 그 결과 실드 안에서 시각적으로 약간 위로 뜬다.
**확정 전에는 심볼을 바꾸지 않았다.** B 나 C 로 가면 심볼·파비콘·마스크아이콘·락업·OG·**명함 30종**이 전부 재생성 대상이다.
