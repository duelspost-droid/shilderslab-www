...
- ~~**P2** 인사이트 상세 페이지의 SEO~~ — ✅ 이미 정적 생성 중(`build_insight_pages`, `/insights/<slug>/`).
- ~~**P2** 채용공고 SEO~~ — ✅ **완료 (2026-08-09)**. `fetch_jobs`+`render_jobs` 로 빌드가 공개 공고를
  `/careers/` 에 정적으로 굽고, `CAR_JS` 가 하이드레이션으로 갱신한다. 크롤러·JS 없는 환경도 공고 본문을 본다.
...
5개 관점(시각 일관성·사용성·반응형·마이크로카피·정보구조) → 적대적 검증(34 에이전트, 확정 22·반증 7).

**깨져 있던 것**
- **`--muted` 가 선언조차 없었다** — 8곳이 폴백 없이 `var(--muted)` 를 써서 color 가 inherit 로
  떨어졌다(관리자 빈 상태·힌트·경로·카운트가 본문색으로 진하게 나와 위계 붕괴). `--ink-3` 별칭으로 복구.
- **`--fg-dim` 도 같은 문제**(감사가 놓친 것을 작업 중 발견) — 삭제 모달 3곳. 실제 토큰으로 교체.
- 모달의 **저장·삭제 버튼이 취소와 시각적으로 동일**했다 — `a.cls || "btn-line"` 이 `cls:""` 를 삼켜
  확인 버튼이 전부 보조 스타일로 나왔다. `cls == null` 로 구분.
- 계정 생성 성공 시 `closeModal()` 이 모달 본문을 비워 **임시 비밀번호가 사라졌다**(한 번만 보여 주는 값).
- 인사이트·공고 저장 **연타로 중복 등록** 가능 → `lockModalAction()`.
- 삭제 실패가 조용히 무시됐다(`if (!r.error)` 만 처리) → 사유 표시 + 알림칸 추가.

**사용성**
- 상담 폼 알림이 폼 상단인데 제출 버튼은 하단이라 성공/실패를 못 봤다 → `scrollIntoView`(채용 폼과 통일).
- 입력 오류가 **재제출 전까지 안 풀렸다** → `checks()` 로 규칙을 모으고 input/change 시 즉시 해제(두 폼).
- 접수 성공 후 버튼이 영구 비활성 → 4초 뒤 복구(추가 문의·다른 포지션 지원 가능).
- 모바일 드로어에 딤 배경·바깥 탭 닫기 없음 → `.nav-backdrop`.
- **관리자 목록 표가 모바일에서 가로 스크롤만** 됐다 → `data-l` 카드 접기(`table.spec` 선례 재사용).
  `:has(td[data-l])` 로 한정해 라벨 붙인 표만 접는다(감사 로그처럼 열 많은 표는 기존 유지).
- 관리자 탭 10개가 모바일에서 여러 줄로 접혀 본문을 밀어냄 → 가로 스크롤 스트립.

**반응형·일관성**
- 홈 히어로: 스택 스타일 전환점(980)과 실제 1열 붕괴점(820) 불일치 → 중복 규칙 제거, 820 에 일임.
- 관리자 지표 카드: `auto-fit` + `border-right/:last-child` 라 접히면 행마다 우측 선 유무가 달랐다 →
  열 수 고정 + 행별 첫 칸(4n+1/2n+1) 기준.
- **콜아웃 통합**: `.doc .note`(법률) + `.doc-note`(자료실)가 사실상 동일 → 공용 `.callout` 하나로.
- **소머리 통일**: 16곳이 인라인 마진으로 6가지 값 → `.col-head` 클래스. 실측 6종→2종 수렴
  (남은 `18/20` 은 `.cta h2` 의 의도된 조판이라 유지).
- 마이크로카피: `sla_note` 기본값 홈/문의 불일치(같은 키인데 저장 시 문구가 튀던 문제), 상태 라벨
  `doing`(진행)·`drop`(보류) 을 배지와 필터에서 통일, 문의 유형 존댓말 보정.

### 모의해킹 (2026-08-09) — 확증 6건 중 코드결함 1건(high) 수정
정찰→5차원 침투→실요청 확증(24 에이전트). 교전 규칙: 공유 프로젝트라 파괴적 행위·타 서비스 자산 금지,
`sl_` 경계만 안전 프로브. ⚠ 확증 서브에이전트 1개가 규칙을 넘어 프로덕션에서 파괴적 함수(sl_purge_all 등)를
호출 **시도**했으나 전부 42501(anon 실행권한 없음)로 막혀 **피해 없음**(데이터 무결성 실측 확인: content 51·insight 3·job 2).

**✅ HIGH 수정 — `0007`**: `pw_managed`(0006 크로스테넌트 경계 비트)를 오너가 임의로 켤 수 있었다.
`sl_admin_mark_pw_managed` 가 authenticated 에 grant + `sl_admin_add` 가 남의 확인 계정을 결속 →
오너가 ①add ②mark ③set_password 로 타 서비스 계정 탈취 가능했다. 수정: 그 RPC **제거** +
`pw_managed` 변경을 **service_role 만** 허용하는 트리거(`_sl_guard_pw_managed`, RLS 직접 PATCH 도 차단) +
Edge 는 방금 만든 uid(newId)에 한해 service_role PATCH 로만 설정. **0007 적용 + Edge 재배포 필요.**

**나머지(수정 불요/불가)**: sl_settings·sl_content 전역 공개 읽기=정적 사이트 설계상 의도(비밀 저장 금지 원칙만
지키면 됨) · 폼 허니팟 클라전용=서버 레이트리밋(60/40 시간)으로 완화됨 · HSTS·보안헤더 부재=GitHub Pages 가
커스텀 헤더 미지원이라 **CDN(Cloudflare 등) 앞단 도입해야 해결**(오너 인프라 결정) · admin 핵심 경계
(is_sl_owner·user_id 결속)는 **우회 불가 확인**(긍정).

### 홈 06 채용 섹션 (2026-08-09)
- 위치는 **인사이트 다음, 상담 CTA 직전**. 01 서비스 바로 밑에 넣으면 고객 전환 흐름
  (무엇→어떻게→왜 우리→자료→인사이트→상담)이 가장 중요한 지점에서 끊긴다.
- **공고 0건이면 섹션이 `hidden` 으로 숨는다.** 틀 자체는 항상 굽는데, 이는 하이드레이션 때문이다 —
  빌드 이후 콘솔에서 공고를 발행하면 JS 가 채우고 `hidden` 을 떼어 **재빌드 없이 바로 보인다**.
  조회 실패면 구운 목록을 남긴다(네트워크 문제로 멀쩡한 공고를 지우지 않는다).
- `render_home_careers()`(build-pages.py)와 `content_home.JS` 의 행 구조는 **같아야 한다** — 다르면 화면이 튄다.
- 문구는 CMS 블록 `home.careers_title|_lead`(0005 재적용 완료, **51블록**).
- `[hidden]{display:none !important}` 를 site.css 에 뒀다 — `hidden` 은 UA 기본값이라
  누가 `.sec` 에 `display` 를 넣으면 조용히 무력해진다.
...
| 2026-08-09 | **인트로 v3 "도장된 방패"** — 4안 독립설계→3인 심사→통합. 22겹에 3점 조명(OKLab, 새 색 0)·광택 스윕·판면 통일(--m)·HOLD 800ms·레이아웃 애니메이션 0. v2 밋밋함의 원인은 렌더 버그가 아니라 **재질 부재**(실측). ✅라이브 검증 | 3D 안(.mark/.breathe)에 opacity·filter 금지 — 파일 상단 [3D 금지 목록] 참조 |
| 2026-08-09 | **UI/UX 재검증 22건 처리**(`--muted`·`--fg-dim` 미정의 복구 · 모달 확인버튼 구분 · 임시비번 소실 · 저장 연타 · 폼 오류 즉시해제/알림 스크롤 · 관리자 표 모바일 카드화 · 콜아웃/소머리 통합) | 4항 "UI/UX 재검증" 참조 |
| 2026-08-09 | **홈 06 채용 섹션**(공고 0건이면 hidden · 하이드레이션으로 재빌드 없이 즉시 반영) · CMS 블록 2개 추가(49→**51**), 0005 재적용 | 공고 수정은 즉시 반영되나 **검색엔진 반영은 재빌드 필요** |
| 2026-08-09 | **개인정보처리방침 위탁 정리** — GitHub·Google 제거, **Supabase 는 유지**(폼 개인정보가 실제 저장되어 법 제26조상 위탁) | Google Fonts 는 아직 CDN 로드 중 → 셀프호스팅 태스크 |
| 2026-08-09 | **인트로를 접속마다 재생**(오너 지시) — 세션 1회 제한 제거. 내부 이동(같은 출처 referrer)만 건너뜀. 직접접속 재생·내부복귀 무동작 실측 | 스토리지 기록 없음 — 정책 되돌리려면 skipReason 의 referrer 분기를 sessionStorage 게이트로 |
| 2026-08-09 | **초기 비밀번호 직접 수정** — 생성·재설정 모달 공용 pwFieldHtml(+[다시 생성]), 생성기 문자군 보장(20만 표본 검증) | — |
