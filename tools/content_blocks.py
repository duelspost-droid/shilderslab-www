...
BLOCKS = [
    ...
    dict(
        ...
        default="쉴더스랩(SHIELDUS LAB)은 2026년에 설립된 정보보호 컨설팅 조직입니다. "
                "인증 기준을 충족시키는 관리체계 컨설팅과, 실제 공격 관점의 기술 진단을 한 팀에서 수행합니다.",
    ),
    dict(
        key="about.view_title", kind="text", section="회사소개", label="01 관점 — 제목",
        hint="줄바꿈이 그대로 반영됩니다.",
        default="보고서는 결과가 아니라\n작업 지시서입니다",
    ),
    dict(
        ...
        default=(
            ...
            "**조치까지 이어지지 않는 구조**입니다. 발견 사항이 담당자의 언어로 번역되지 않고, 우선순위가 없고, "
    ...
    dict(
        ...
        default=(
            ...
            "보고서는 두꺼웠지만 **무엇부터 해야 할지는 알 수 없었다**는 것입니다.\n\n"
    ...
    dict(
        ...
        hint="줄바꿈이 그대로 반영됩니다.", default="여섯 개 영역,\n하나의 기준",
    ),
    dict(
        key="services.hero_lead", kind="text", section="서비스", label="상단 소개 문단",
        hint="서비스 목록 위에 놓이는 한 문단입니다.",
        default="관리체계·기술진단·개인정보·클라우드를 서로 다른 언어로 다루면 조치가 흩어집니다. "
                "모든 발견 사항을 같은 위험도 기준과 우선순위로 정리해 전달합니다.",
    ),
    # ─────────────────────────── 푸터 ───────────────────────────
    dict(
        key="footer.blurb", kind="text", section="푸터", label="푸터 소개 문단",
        hint="모든 페이지 하단에 공통으로 나옵니다.",
        default="규제 대응과 공격자 관점의 기술 진단을 한 팀에서 수행하는 정보보호 컨설팅 조직입니다. "
