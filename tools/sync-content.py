...
🚨 이 도구는 **코드 → DB** 한 방향이다. 반대로 끌어오지 않는다.

  생성되는 UPDATE 는 `where value = <생성 시점의 DB 값>` 조건을 단다.
  이것이 막아 주는 것은 **SQL 을 만든 뒤 적용하기 전까지 사이에 생긴 변경**뿐이다.
  **이미 오너가 콘솔에서 고쳐 둔 문구는 그대로 덮어쓴다.** 조건절이 바로 그 값과 일치하기 때문이다.
  (예전 주석은 "절대 덮어쓰지 않는다" 고 적혀 있었다. 사실이 아니어서 2026-08-08 바로잡았다.)

  아래 리포트의 "차이"는 이제 대부분 **오너가 콘솔에서 고쳤다는 뜻**이다.
  코드 쪽 문구를 고친 게 아니라면 SQL 을 만들지 말고 그대로 두라.
  DB 가 정본이고, 빌드는 DB 값을 구워 넣는다.
